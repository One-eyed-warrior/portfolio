...
function commandHandler(input : string) {
  if(input.startsWith("rm -rf") && input.trim() !== "rm -rf") {
    if (isSudo) {
      if(input === "rm -rf src" && !bareMode) {
        ...
        easterEggStyles();
        setTimeout(() => {
          writeLines(["rm -rfing on a random website you just dug up?", "<br>"]);
        }, 200)

        setTimeout(() => {
          writeLines(["Congrats, you have single handedly messed it up", "<br>"]);
        }, 1200)
  ...
  switch(input) {
    ...
    case 'banner':
      if(bareMode) {
        writeLines(["SidShell v1.0.42", "<br>"])
        break;
      }
      writeLines(BANNER);
      break;
    case 'help':
      if(bareMode) {
        writeLines(["try restarting your browser", "<br>"])
        break;
      }
      writeLines(HELP);
      break;
